...
未完成真人语料、Release 模型生命周期及原生交互验收前，不默认启用实验性能策略。
...
热模式在每次计分前执行一次有记录约定的同音频非计分推理。失败行保留，失败退出码为 2；
...
该工具不测预览、保存、粘贴、完整模型池闲置恢复或峰值内存；缺失项保持缺失，
因此单靠回放报告不能通过完整产品性能门槛。词汇与 LLM 阶段也不由此工具代跑。
