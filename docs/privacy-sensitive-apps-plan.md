...
> 实施状态（2026-07-18）：核心运行时闸门、无确认副作用的动态隐私解释、系统保护 pasteboard 检测、Secure Input、云端确认、Deepgram 实时会话持续授权与关流、sherpa-onnx 固定归档准备与离线识别披露、历史与活动流三态预览、Keychain 凭据与 Webhook 配置迁移、自定义敏感 App 管理、暂停捕获、忽略下一次复制、隐私设置原子快照、本地历史/诊断留存、本地静态加密、可选失败录音恢复、临时音频安全清理，以及来源 App 感知的剪贴板 Replay/Replace 授权均已完成。
...
- `SettingsView` 已有权限、sherpa-onnx 本地语音与 Deepgram 云端语音设置入口。
...
4. 本地/云端路径提示与运行前确认；Deepgram 音频及匹配当前作用域的 Nova-3 keyterms 都只会在确认后进入请求快照。旧 Webhook 端点与请求头会先写入 Keychain 并精确回读，再以引用原子替换 SQLite 明文并清理 DB/WAL 残留；迁移失败时只隔离自定义工作流。Webhook 在发布版本仍隐藏且不注册。
...
11. 延迟音频使用不可读、一次性的 run/workflow-bound lease：解析前 claim，解析后、recognizer 前 final revalidate。Deepgram live 额外持有同 run lifetime：permit 获取与 revoke 原子线性化，焦点或设置变为受限时停止输入、丢弃未发送缓冲并关闭 socket；输入停止后再次 seal，未 seal、已撤销或 provider 失败的会话不能进入录后整段 fallback。已经进入系统网络调用的单个 chunk 无法撤回，但撤销线性化后不会再授予新的发送 permit。
...
| 云端处理确认 | 开 | 使用 Deepgram/未来云端 LLM 前显示本次音频/文本会离开本机 |
...
- Deepgram cloud：音频会发送到 Deepgram API；Nova-3 还会收到匹配当前 App、剪贴板组和语言作用域的 keyterms。二者都在本次云端确认后才准备和发送；转写结果会回到本机，并按本地留存策略进入运行历史或剪贴板组。
...
```text
当前路径：云端识别
录音音频以及匹配当前作用域的 Nova-3 热词会发送到 Deepgram。两者都在确认后才进入请求；识别结果会按你设置的留存期限保存在本机。敏感排除规则命中时不会捕获对应剪贴板或选中文本。
```
...
- Deepgram selected 时，policy 返回 `requireCloudConfirmation`。
- 诊断 metadata 不包含敏感文本。
