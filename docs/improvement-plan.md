...
| P1-1 应用状态 | 已删除 40 个状态转发属性，另迁移 82 项状态/任务并移除 21 处 didSet；设置验证、隐私写入、字幕计时、工作流解释、词库编辑及语音准备任务归功能模型；查询命令已显式化；剩余设置加载与跨功能协调继续收敛 | 功能模型拥有状态和任务；显式命令；完整生产依赖；组合根只组装 |
...
| P3 门禁 | 全 target 导入检查与隔离领域测试已实现；整合主线后的完整门禁已通过；后续切片继续验证 | 确定性测试、全模块边界、产品基准、隔离快速测试与完整 CI |
