...
选中记录后，点击快捷面板的“预览”（也可右键选择）查看内容；打开后会随所选记录更新。
主窗口的记录详情使用相同预览。图片可“查看大图”；文件显示缩略图、类型和大小，点击
文件旁的预览按钮使用 macOS Quick Look，多文件记录可在预览窗口中前后切换。
文件引用显示原路径当前的内容，不保存文件副本；文件移动、删除或无法读取时会明确提示。
...
有 TypeSafe API Key 时，可点“用 Jev 比较…”：从当前关键词和语义候选中交错选取最多 10 条，
先预览查询及将要发送的文字片段／文件名，再主动提交给 `jev-1.13.0` 付费评分。
Key 只保留在本次 App 会话的内存中，退出即清除。评分不自动运行、不改变本地次序；
选择结果只定位记录，粘贴仍由你确认。图片和引用文件的正文不发送，敏感来源会阻止请求。
每条候选最多发送 1,800 UTF-8 字节，界面标明截短；失败不自动重试，已发送的请求仍可能计费。

“所有记录”展示仍在保留期内的记录。收件箱、语音输入及自建记录集用于组织内容，
同一条记录可以属于多个记录集。待输出内容由独立容器保存：剪贴板使用 Stack，
语音收集模式使用 Queue；直接输入成功的语音不会再次排队。

按 **⌘⇧V** 输出下一项，原有打开剪贴板面板的快捷键保持不变。
组间优先选择含最新未消费输入的容器，组内按 Stack 最新优先或 Queue 最早优先输出。
待用内容没有时间窗口，重启后仍可继续。Set 用于手动取用，同一记录只加入一次，使用后保留。
在记录页的“待输出容器”菜单中，可以启用旧容器、加入历史记录或创建 Set。
若复制内容已存入历史但未能加入容器，Rill 会提示失败并保留记录；选中该记录后，
可通过同一菜单的“加入”重试，无需再次复制。
迁移的旧容器默认关闭，新默认容器从空开始；Raycast 历史不会自动加入。

特殊输出不写入系统通用剪贴板。文本通过辅助功能或 Unicode 键盘事件插入；
无法验证结果时，需要确认“已插入”后才推进，或选择重试并重新聚焦目标。
图片和文件通过浮层拖到目标应用，仅复制原文件。失败、取消及部分输出保留条目；
目标应用的支持情况以实际交互为准，Rill 不自动提交表单或发送消息。
“输出下一项”快捷键可以在设置中修改，并检查与面板快捷键的冲突。

旧工作流的记录集路由继续保留以下预设，它们不参与新的“输出下一项”调度：
...
30 天，可在设置中调整和分类清理；待输出容器中的记录受批量清理保护，可以明确删除
未在输出或等待确认中的单条记录。失败录音恢复默认关闭，开启后最多保留 24 小时。
