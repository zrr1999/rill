...
class BuildDriverTests(unittest.TestCase):
    ...
    def test_unchanged_environment_does_not_clean(self):
        ...
        with patch.object(self.context, 'swift'), patch.object(self.context, 'clean') as clean:
    ...
    def test_product_and_filter_do_not_invalidate_environment(self):
        self.assertEqual(build.build_settings(['--product', 'RillApp', '--filter', 'OneTest']), [])
        self.assertNotEqual(build.build_settings(['-Xswiftc', '-O']), [])

    def test_stale_cache_retries_once_but_compile_error_does_not(self):
        with patch.object(self.context, 'prepare'), patch.object(self.context, 'clean') as clean:
            with patch.object(self.context, 'swift', side_effect=[
                ...
                clean.assert_called_once()
            clean.reset_mock()
            with patch.object(self.context, 'swift', side_effect=build.BuildFailure(1, 'type error')):
    ...
    def test_build_and_test_retain_locked_dependency_policy(self):
        ...
        self.assertEqual(command[:5], ['swift', 'test', '--force-resolved-versions', '-Xswiftc', '-warnings-as-errors'])
    ...
    def test_lock_blocks_second_process_and_release_recovers(self):
        ...
        with self.context.lock():
            process = multiprocessing.Process(target=try_lock, args=(str(self.context.lock_path), sender))
            process.start()
    ...
    def test_receipt_preserves_products_and_rejects_changed_inputs(self):
        ...
        build.make_receipt(self.context, output, receipt_path, build.source_inputs(self.root))
