...
public final class PasteboardController: Sendable {
    ...
    /// pasteboard. These limits are separate from ClipboardStorageLimits: the
    ...
    public enum ConditionalWriteError: Error, Sendable, Equatable {
        case protectedClipboard([ClipboardProtection])
    ...
    /// ClipboardSnapshot intentionally remains a policy/history projection and
    ...
    private let storageLimits: ClipboardStorageLimits
    ...
    public func currentSnapshot() async -> ClipboardSnapshot {
    ...
    public func privacySafeSnapshot() async -> ClipboardSnapshot {
        let descriptor = currentDescriptor()
    ...
    public func currentDescriptor() -> ClipboardDescriptor {
        let availableTypes = Set(pasteboard.types ?? [])
        return ClipboardDescriptor(
    ...
    public func readSnapshot(ifChangeCountIs expected: Int) async -> ClipboardSnapshot? {
    ...
    public func beginTemporaryWrite(
        _ replacement: ClipboardSnapshot,
    ...
    static func protections(forPasteboardTypeNames typeNames: Set<String>) -> [ClipboardProtection] {
        var protections: [ClipboardProtection] = []
    ...
    private struct ImageReadResult: Sendable {
        let data: Data?
        let rejection: ClipboardStorageRejectionReason?
    }
    ...
    private static func encodedMetadata(
        for captureTags: [ClipboardCaptureTag],
        writerInstanceID: UUID,
        limits: TemporaryPreservationLimits
    ) -> Data {
...
/// A pure ImageIO boundary kept outside PasteboardController's MainActor
...
private enum PasteboardImageIngress {
    ...
    private static func validatedSource(
        ...
        declaredFormat: DeclaredFormat,
        limits: ClipboardStorageLimits
    ) -> Swift.Result<CGImageSource, ClipboardStorageRejectionReason> {
