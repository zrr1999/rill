...
public actor RecordStore {
    ...
    public func collection(id: RecordCollectionID) async throws -> RecordCollectionProjection? {
        ...
        return RecordCollectionProjection(
            collection: collection, memberships: memberships, recordsByID: records)
    }
    ...
    public func ingest(
        ...
        guard membershipsByID.count + destinations.count <= RecordGraphLimits.maximumMemberships
        else {
    ...
    public func deleteRecord(_ recordID: RecordID) async throws {
        ...
        guard !reuseLeases.values.contains(where: { $0.record.id == recordID }) else {
            throw RecordStoreError.membershipAlreadyInUse
        }
    ...
    public func replace(
        ...
        let destinationMemberships =
            inAllCollections
            ? membershipIDsByRecordID[sourceRecord.id, default: []].compactMap {
                membershipsByID[$0]
            }
            : [sourceMembership]
    ...
    public func deletionImpact(for collectionID: RecordCollectionID) async throws
        -> RecordCollectionDeletionImpact
    {
        try await ensureInitialized()
        ...
        return RecordCollectionDeletionImpact(
            ...
            captureRuleIDs: captureRules.filter {
                $0.destinationCollectionIDs.contains(collectionID)
            }
            .map(\.id),
        ...
        for collectionID in sourceIDs {
            ...
            guard collection.selectionPolicy != .manual,
                let membership = selectedMembership(
                    from: active, policy: collection.selectionPolicy),
            ...
            return RecordRouteProjection(
                ...
                previewText: record.payload.textValue.map { text in
                    ...
                    return normalized.count <= 120
                        ? normalized : String(normalized.prefix(117)) + "…"
                },
    ...
    public func completeDelivery(
        ...
        if let reuse = reuseLeases[leaseID] {
            guard receipt == nil, settlingLeaseIDs.insert(leaseID).inserted else {
                throw RecordStoreError.invalidDeliveryReceipt
            }
            defer { settlingLeaseIDs.remove(leaseID) }
            guard var activity = activityByRecordID[reuse.record.id] else {
                throw RecordStoreError.recordUnavailable
            }
            ...
            return RecordDeliveryReceipt(
                recordID: reuse.record.id, membershipID: nil, sink: reuse.sink,
    ...
    private func addMembershipWithoutPersistence(
        ...
        guard
            membershipIDsByRecordID[recordID, default: []].count
    ...
    private func projection(for recordID: RecordID) async throws -> RecordProjection? {
        ...
        return RecordProjection(
            ...
            memberships: membershipIDsByRecordID[recordID, default: []].compactMap {
                membershipsByID[$0]
            }
        )
    }
    ...
    private func makeCatalogSnapshot() -> RecordCatalogSnapshot {
        RecordCatalogSnapshot(
            revision: revision, records: recordOrder.compactMap(summary),
            ...
            capacity: RecordCapacity(
                count: recordsByID.count, byteCount: totalPayloadByteCount(), limits: storageLimits,
                admissionWasLimited: admissionWasLimited))
    }
    ...
    private func activeMembershipCount(in collectionID: RecordCollectionID) -> Int {
        membershipIDsByCollectionID[collectionID, default: []].reduce(into: 0) {
            count, membershipID in
    ...
    private func validateRecordAdmission(
        ...
        guard totalPayloadByteCount() <= storageLimits.maximumTotalPayloadByteCount - payloadBytes
        else {
            admissionWasLimited = true
            publishSnapshotToObservers()
...
extension RecordStore {
    ...
    private func install(_ legacy: LegacyClipboardMigration.MigratedGraph) throws {
        let headers = try legacy.records.map {
            RecordHeader(record: $0, byteCount: try encodedPayload($0.payload).count)
        }
        try install(
            InstalledGraph(
    ...
    private func install(_ graph: InstalledGraph) throws {
        ...
        let membershipCounts = Dictionary(grouping: graph.memberships, by: \.recordID).mapValues(
            \.count)
        guard
            membershipCounts.values.allSatisfy({
                $0 <= RecordGraphLimits.maximumMembershipsPerRecord
            })
        else {
            throw RecordStoreError.invalidGraph
        }
        ...
        metadataByRecordID = Dictionary(
            uniqueKeysWithValues: graph.metadata.map { ($0.recordID, $0) })
        activityByRecordID = Dictionary(
            uniqueKeysWithValues: graph.activity.map { ($0.recordID, $0) })
        ...
        for collectionID in collectionsByID.keys
        where membershipIDsByCollectionID[collectionID] == nil {
    ...
    private func persistCurrentGraph() async throws {
        ...
        guard let persistence else {
            if let rollbackState, recordsByID.count < rollbackState.recordsByID.count {
                admissionWasLimited = false
            }
            trimPayloadCache()
            committedGraphState = captureCommittedGraphState()
            publishSnapshotToObservers()
        ...
        do {
            if let catalogStore = persistence as? any RecordCatalogPersistenceStore {
                ...
                repositoryRevision = try await persistence.replaceRecordGraph(
                    with: prepared.snapshot)
    ...
    private func preparePersistenceWrite() throws -> PreparedPersistenceWrite {
        ...
        for recordID in recordOrder {
            ...
            if let durable = durableBlobReferencesByRecordID[recordID] {
                ...
                guard let content = payloadCache[record.id] else {
                    throw RecordStoreError.recordUnavailable
                }
    ...
    private func materializeRecords(
        ...
        let blobsByID = Dictionary(
            uniqueKeysWithValues: payloadBlobs.map { ($0.reference.blobID, $0) })
...
extension RecordStore {
    fileprivate func collectionEvent(
    ...
    fileprivate func publishCollectionEvents(_ descriptors: [RecordCollectionEventDescriptor]) async
    {
...
extension RecordStore {
    fileprivate func materializedRecord(_ id: RecordID) async throws -> Record? {
        ...
        guard let repository = persistence as? any RecordCatalogPersistenceStore,
            let reference = durableBlobReferencesByRecordID[id]
        else {
            throw RecordStoreError.persistenceUnavailable
        }
    ...
    fileprivate func cachePayload(_ payload: RecordPayload, for id: RecordID) {
    ...
    fileprivate func trimPayloadCache() {
    ...
    func evictPayloadsOverBudget() {
        ...
        while payloadCacheBytes > 64 * 1_024 * 1_024,
            let index = payloadCacheOrder.firstIndex(where: {
                durableBlobReferencesByRecordID[$0] != nil
            }
            )
        {
    ...
    fileprivate func installCatalog(_ catalog: RecordCatalogRead) throws {
        func values<T: Decodable>(_ kind: RecordCatalogNode.Kind, _ type: T.Type, id: (T) -> String)
            throws -> [T]
        {
        ...
        let references = Dictionary(
            uniqueKeysWithValues: catalog.references.map { ($0.recordID, $0) })
        ...
        try install(
            InstalledGraph(
                records: headers,
                metadata: try values(
                    .metadata, RecordMetadata.self, id: { $0.recordID.description }),
                activity: try values(
                    .activity, RecordActivity.self, id: { $0.recordID.description }),
                collections: try values(
                    .collection, RecordCollection.self, id: { $0.id.description }),
                memberships: try values(
                    .membership, RecordMembership.self, id: { $0.id.description }),
                captureRules: try values(
                    .captureRule, CaptureRouteRule.self, id: { $0.id.description }),
                deliveryRules: try values(
                    .deliveryRule, DeliveryRouteRule.self, id: { $0.id.description }),
        ...
        guard Set(catalog.manifest.collectionOrder) == Set(collectionOrder),
            catalog.manifest.collectionOrder.count == collectionOrder.count
        else { throw RecordStoreError.invalidGraph }
    ...
    func prepareCatalogMutation() throws -> (mutation: RecordCatalogMutation, references: [RecordID: RecordGraphPersistenceBlobReference]) {
        ...
        for id in touchedRecords where recordsByID[id] != nil && references[id] == nil {
            ...
            let reference = RecordGraphPersistenceBlobReference(
                blobID: UUID(), recordID: id, kind: header.kind, byteCount: data.count)
        ...
        let mutation = RecordCatalogMutation(
            ...
            manifest: RecordCatalogManifest(
                nextMembershipOrdinal: nextMembershipOrdinal, recordOrder: recordOrder,
                collectionOrder: collectionOrder),
            upserts: nodes, removedKeys: removedKeys, newPayloadBlobs: blobs,
            removedPayloadBlobIDs: removedBlobs
        )
...
extension RecordStore {
    public func query(_ query: RecordQuery, offset: Int = 0, limit: Int = 50) async throws
        -> RecordQueryPage
    {
        try await ensureInitialized()
        ...
        while index < batchEnd, results.count < pageSize {
            ...
            guard let item = summary(for: id),
                ...
                query.sourceBundleIdentifier == nil
                    || item.header.provenance.sourceBundleIdentifier
                        == query.sourceBundleIdentifier,
                query.collectionID == nil
                    || item.memberships.contains(where: { $0.collectionID == query.collectionID })
            else { continue }
        ...
        return RecordQueryPage(
            revision: revision, records: results, nextOffset: index < ids.count ? index : nil)
    }
    ...
    public func prepareCleanup(olderThan cutoff: Date = .distantFuture) async throws
        -> RecordCleanupPlan
    {
        try await ensureInitialized()
        let candidates = recordOrder.reversed().filter { id in
            guard !reuseLeases.values.contains(where: { $0.record.id == id }),
                ...
                let metadata = metadataByRecordID[id], !metadata.isPinned, metadata.tags.isEmpty
            else { return false }
            return membershipIDsByRecordID[id, default: []].allSatisfy { membershipID in
                ...
                return membership.collectionID == RecordCollection.inboxID
                    && !leasedMembershipIDs.contains(membershipID)
            }
        }
        return RecordCleanupPlan(
            revision: revision, recordIDs: candidates,
    ...
    public func prepareCleanup(scope: RecordCleanupScope) async throws -> RecordCleanupPlan {
        ...
        return RecordCleanupPlan(
            revision: revision, recordIDs: recordIDs,
    ...
    public func confirmCleanup(
        ...
        case .collection(let id):
            ...
            return RecordCleanupResult(
                removedCount: 0, preservedActiveCount: eligible.protectedCount)
        case .membership(let id):
        ...
        case .record(let id):
            ...
            return RecordCleanupResult(
                removedCount: 1, preservedActiveCount: eligible.protectedCount)
        case .history: break
        }
        ...
        try await persistCurrentGraph()
        return RecordCleanupResult(
            removedCount: removed.count, preservedActiveCount: recordsByID.count)
    }
}
...
extension RecordStore {
    public func beginReuse(_ subject: RecordReuseSubject, sink: RecordSinkIdentity) async throws
        -> RecordReuseLease
    {
        try await ensureInitialized()
        guard sink != .recordCollection,
            let record = try await materializedRecord(subject.recordID),
            metadataByRecordID[subject.recordID]?.revision == subject.metadataRevision
        else {
            throw RecordStoreError.recordUnavailable
        }
