...
@MainActor
public protocol SystemClipboardAccess: Sendable {
...
extension SystemClipboardPort: SystemClipboardAccess {
  public func currentClipboardChangeCount() async -> Int {
    currentChangeCount()
  }

  public func currentClipboardDescriptor() async -> SystemClipboardDescriptor {
    currentDescriptor()
  }
  ...
  public func ownsClipboardChangeCount(_ changeCount: Int) async -> Bool {
...
public actor SystemClipboardCaptureController {
  ...
  private let recordStore: any SystemClipboardRecordCapturing
  public nonisolated let recordDelivery: RecordInteractionController
  ...
  public init(
    ...
    recordStore: any SystemClipboardRecordCapturing,
    delivery: any RecordDeliveryPerforming,
    eventBus: any RillEventPublishing,
  ...
  init(
    ...
    self.recordDelivery = RecordInteractionController(
      delivery: delivery, eventBus: eventBus,
      focus: self.focusIdentitySampleProvider, accessibility: accessibilityChecker,
      permissionFailureMessage: TextInjectionEngine.InjectionError.accessibilityPermissionRequired
  ...
  public func stop() async {
    ...
    await recordDelivery.stopAcceptingRequests()
  ...
  private func pollExternalClipboardIfNeeded(at now: ContinuousClock.Instant = .now) async {
    ...
    guard
      changeCount != lastObservedPasteboardChangeCount
...
extension SystemClipboardCaptureController {
  fileprivate func captureExternalClipboardIfNeeded(at now: ContinuousClock.Instant = .now) async {
    ...
    guard
      descriptor.changeCount != lastObservedPasteboardChangeCount
    ...
    let byteCount =
      snapshot.plainText.utf8.count + (snapshot.imagePNGData?.count ?? 0)
