...
extension UIStrings {
    ...
    static func workflowExplanationOutput(
        ...
        case (.english, .clipboardHistoryWrite): "Save clipboard history"
        case (.simplifiedChinese, .clipboardHistoryWrite): "保存剪贴板历史"
        case (.english, .focusedApplicationWrite): "Write focused application"
        case (.simplifiedChinese, .focusedApplicationWrite): "写入当前应用"
        case (.english, .deliveryStackWrite): "Save delivery stack"
        case (.simplifiedChinese, .deliveryStackWrite): "保存到投递栈"
