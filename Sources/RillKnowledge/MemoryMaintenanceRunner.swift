...
public struct MemoryMaintenanceSession: Sendable {
    ...
    public init(
        authorization: ContextReferenceAuthorization, workflowIDs: Set<UUID>,
        excludedApplications: Set<String> = [], consolidator: any MemoryConsolidating
    ) {
...
public actor MemoryMaintenanceRunner {
    ...
    public init(
        repository: any ContextMemoryRepository, eligible: @escaping @Sendable () async -> Bool,
    ...
    public func runIfEligible() async {
        guard !stopped, work == nil, await eligible(), let session = await session(),
            session.authorization.isValid, !stopped, work == nil
        else { return }
        let id = UUID()
        operationID = id
