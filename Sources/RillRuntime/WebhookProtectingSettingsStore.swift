    ...
    public func migrateIfNeeded() async -> WebhookConfigurationMigrationResult {
        ...
        for (workflowIndex, workflow) in workflows.enumerated() {
            for (actionIndex, action) in workflow.pipeline.outputActions.enumerated() {
                ...
                guard let expectedReference = WebhookConfigurationReference(
                ...
                let resolvedReference = storedReference ?? (
                    protectedConfiguration == nil ? nil : expectedReference
                )
        ...
        for plan in plans {
            ...
            do {
                guard try await secureStore.configuration(for: plan.reference) == plan.protectedConfiguration else {
        ...
        let requiresPurge = !plans.isEmpty
...
public enum WebhookProtectingSettingsStoreError: Error, LocalizedError, Sendable, Equatable {
    ...
    public var errorDescription: String? {
        switch self {
        case .workflowLibraryLocked:
            return "Workflow library writes are locked until Webhook configuration protection succeeds."
        case .reservedProtectionState:
...
public actor WebhookProtectingSettingsStore: SettingsStore {
    ...
    public func strings(forKeys keys: [AppSettingKey]) async throws -> [AppSettingKey: String] {
        let result = keys.contains(where: { Self.requiresProtection(for: $0) })
