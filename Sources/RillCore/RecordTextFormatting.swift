...
public enum RecordTextFormatting {
    ...
    public static func previewText(_ text: String, limit: Int = 280) -> String {
        guard limit > 0 else { return "" }
        let summarized = summaryText(text)
        guard summarized.count > limit else { return summarized }
        guard limit > 1 else { return "…" }

        let truncated = summarized
            .prefix(limit - 1)
