...
final class SQLitePersistenceStoreTests: XCTestCase {
    ...
    async throws
  {
    ...
    do {
      ...
      for blockedWrite in blockedWrites {
        ...
        XCTAssertTrue(
          message.contains(SQLiteWriterBarrier.capabilityFunctionName),
  ...
  func testRunHistoryContentAccessAvoidsBodyOpensAndCapsRestrictedProjection() async throws {
    ...
    XCTAssertNil(restricted.correctionSource)
    XCTAssertFalse(protector.openedContexts().contains { $0.field == "correction_source_json" })
